...
Abyss는 Darkest Dungeon의 전투·탐험·영지 시스템을 Unity와 C#으로 재구현한 프로젝트다. 전투·탐험·성장 규칙을 일반 C# 어셈블리에 두고, Unity는 상태 표시와 입력 연결을 맡도록 구성했다. 동일한 규칙을 콘솔 실행과 Unity 클라이언트에서 사용한다.
